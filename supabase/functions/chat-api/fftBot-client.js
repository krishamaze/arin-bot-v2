...
(function() {
  ...
  const CFG = {
    EDGE_URL: 'https://opaxtxfxropmjrrqlewh.supabase.co/functions/v1/chat-api',
    BUCKET_DELAY: 7000,
    MAX_BUCKET: 10,
    INIT_CMD: 'hi/',
    DEBUG: true
  ...
  const detectMsg = async (el) => {
   ...
   if (BUCKET.events.length >= CFG.MAX_BUCKET) {
  log('🚀 Bucket full, sending immediately');
  ...
  const sendBucket = async () => {
    if (!STATE.enabled || BUCKET.processing || BUCKET.events.length === 0) return;
        ...
        const hasUserMessages = BUCKET.events.some(e => 
          e.platformId !== STATE.platformId &&
      e.type !== 'system'
    );

    if (!hasUserMessages) {
      log(`⏳ System/bot events only, postponing ${CFG.BUCKET_DELAY}ms`);
      BUCKET.processing = false;
      BUCKET.timer = setTimeout(resetTimer, CFG.BUCKET_DELAY);
      return;
    }
